# CueLine — Design & Implementation Document

## Overview

CueLine is a mobile app for actors learning lines. It is, at its core, a **sophisticated audio player with structured navigation**: it plays pre-recorded audio cues, waits for the actor to speak their line from memory, and on demand plays the correct reference recording of that line so the actor can self-assess.

The key distinction from existing apps (Rehearsal Pro, coldRead, etc.) is that CueLine does **not** require the actor to record anything. Instead, a **scriptpack** — a zip archive containing professional audio recordings of every line, cue, and direction, plus a structured JSON manifest — is prepared by the production and distributed to the cast. The actor downloads it and begins rehearsing immediately.

This makes CueLine suitable for theatre companies, drama schools, film productions, and publishers who want to supply a polished rehearsal resource alongside a script.

## Relationship To This Project

This document describes the mobile app product. It is not the source of truth for the package manifest generated by this repository.

The Androcles codebase already has a structured play model:

- `Play`, `Role`, and `Part` in `src/play.py`.
- `BlockId` values using `part.block` identity, e.g. `0.2`.
- `SegmentId` values used for audio filenames, e.g. `0_2_1.wav`.
- Role segment audio under `build/<play>/audio/segments/<ROLE>/<segment_id>.wav`.
- Narrator, caller, and announcer as special roles: `_NARRATOR`, `_CALLER`, `_ANNOUNCER`.
- A planned app package contract in `planning/app_manifest.md`.

Use `planning/app_manifest.md` as the authoritative manifest schema. This CueLine document should stay focused on app behavior: session flow, navigation, voice commands, screens, and persistence.

---

## Core Principles

- **Audio-first, eyes-free.** The primary use case is rehearsal while driving or doing other tasks. All essential functions must be reachable by voice command.
- **No backend required at runtime.** After the scriptpack is downloaded, the app operates entirely offline. No accounts, no sync, no network dependency.
- **Self-assessment, not algorithmic scoring.** The actor hears their cue, speaks their line, then optionally hears the correct version to judge themselves. The app does not evaluate performance.
- **Simple state machine.** The app walks a flat, ordered list of rehearsal lines for the selected role. Navigation is the primary complexity; playback is straightforward.

---

## Technology Stack

This stack is provisional product planning, not part of the Androcles hardening work. Validate current library capabilities before starting the mobile app.

| Layer | Choice | Rationale |
|---|---|---|
| Framework | React Native (Expo) | Single codebase for iOS and Android; strong audio and filesystem support |
| Audio playback | `expo-av` | Plays local audio files; supports background audio sessions |
| Voice commands | `@react-native-voice/voice` or equivalent | Small command vocabulary; exact offline/on-device behavior must be verified per platform |
| Local storage | MMKV | Fast key-value store for session state, progress, bookmarks |
| File handling | `expo-file-system` + `expo-document-picker` | Unzipping scriptpacks, accessing local audio files |
| Navigation | React Navigation | Standard stack + tab navigation |
| Distribution | App Store + Google Play | Scriptpacks distributed separately (static file host, CDN, or direct transfer) |

**No server is required beyond a static file host** (e.g. S3, Cloudflare R2, or even a shared drive) for scriptpack distribution. There is no auth, no API, no user database.

---

## Scriptpack Format

A scriptpack is a `.zip` file distributed by the production. The actor downloads it once; it then lives on-device.

### File structure

```
<play-id>/
├── manifest.json
└── audio/
    ├── segments/
    │   ├── CHRISTINE/
    │   │   ├── 0_2_1.wav
    │   │   └── ...
    │   └── _NARRATOR/
    │       ├── 0_1_1.wav
    │       └── ...
    └── callouts/
        ├── CHRISTINE_callout.wav
        └── ...
```

Initial Androcles-generated packages should keep segment audio as WAV because that is the existing split-output format. A later packaging phase may add an app-optimized transcode step, likely M4A/AAC, but that should be a deliberate package-builder feature rather than an app assumption.

### manifest.json schema

The manifest schema is defined in `planning/app_manifest.md`.

The mobile app should consume roles and rehearsal line items, not a single flat `snippets` array. A simplified shape is:

```json
{
  "schema_version": 1,
  "play": {
    "id": "fairies",
    "title": "The Curious Case of the Cottingley Fairies"
  },
  "roles": [
    {
      "id": "CHRISTINE",
      "display_name": "Christine",
      "lines": [
        {
          "id": "0_2_CHRISTINE",
          "block_id": "0.2",
          "cue": {
            "speaker": "_NARRATOR",
            "text": "1966.",
            "audio": {
              "path": "audio/segments/_NARRATOR/0_1_1.wav",
              "duration_ms": 1200,
              "required": false
            }
          },
          "response": {
            "text": "This is Christine Canfield with the London Sun...",
            "segments": [
              {
                "id": "0_2_1",
                "audio": {
                  "path": "audio/segments/CHRISTINE/0_2_1.wav",
                  "duration_ms": 4200,
                  "required": true
                }
              }
            ]
          }
        }
      ]
    }
  ]
}
```

Important manifest rules:

- Roles come from the parsed Androcles `Play.roles`, excluding `_NARRATOR`, `_CALLER`, and `_ANNOUNCER` from actor role selection by default.
- Line ids are based on existing block and segment ids, not newly assigned sequential ids.
- Direction and narrator cues may include `_NARRATOR` audio when available.
- Required response audio is expected to exist; missing required response audio should fail package generation.
- Optional cue audio may be missing if cue text is available, unless a stricter package mode requires complete playable cues.

The app may derive a flat session sequence from `roles[].lines` after the actor selects a role.

---

## App State Model

The app's session state is a simple cursor over the selected role's line array.

### Session configuration (set before starting)

```
selectedRole: string             // e.g. "CHRISTINE"
cueDepth: number                 // how many preceding cue items to play when available (default: 1)
includeDirections: boolean       // whether direction cues are played when narrator audio exists
startLineId: string              // where to begin
```

### Session state (persisted to MMKV)

```
currentLineId: string            // the actor's NEXT line to attempt
playbackMode: enum               // CUE_PLAYING | WAITING | REFERENCE_PLAYING | PAUSED
```

### Derived values (computed from manifest + session config)

- **My lines**: `roles[].lines` for `selectedRole`.
- **Cue for line N**: cue payloads attached to the selected line, optionally expanded by `cueDepth` if the manifest includes enough preceding context.
- **Next/previous**: walking the selected role's line array.

---

## Session Flow

```
START
  │
  ▼
Play cue(s)                  ← one or more audio files played sequentially
  │
  ▼
WAITING                      ← silence; actor speaks their line from memory
  │                            app listens for voice commands only
  ├── "hear my line"  ──────► Play reference audio for current line
  │                              │
  │                              └── returns to WAITING
  │
  ├── "next" / "go"  ────────► advance to next actor line → Play cue(s)
  │
  ├── "again" / "repeat" ───► replay cue(s) without advancing
  │
  ├── "back" ───────────────► go to previous actor line → Play cue(s)
  │
  ├── "skip" ───────────────► advance without hearing reference → Play cue(s)
  │
  └── "stop" / "pause" ─────► PAUSED (session state saved)
```

The app **never auto-advances**. It always waits for an explicit command before moving on. There is no timeout.

---

## Voice Commands

Voice recognition runs continuously in the background during a session (listening for commands only, not scoring lines). The vocabulary is small enough that simple keyword matching suffices — no NLP needed.

| Utterance(s) | Action |
|---|---|
| "next", "go", "continue" | Advance to next actor line and play cue |
| "again", "repeat", "cue again" | Replay current cue without advancing |
| "hear my line", "correct", "play my line" | Play reference audio for current line |
| "back", "previous" | Go to previous actor line and play cue |
| "skip" | Advance to next line without playing reference |
| "go to scene [N]" | Jump to beginning of scene N |
| "go to act [N]" | Jump to beginning of act N |
| "stop", "pause" | Pause session and save state |
| "start", "resume" | Resume from saved state |

**Implementation note:** STT sessions on iOS (`SFSpeechRecognizer`) have a ~1-minute audio limit. Restart the session in a loop. On Android, recognition quality varies by device; lower-end devices may silently fall back to network STT — document this in the app's setup flow.

Voice commands are a convenience layer. All commands must also be reachable by on-screen tap, so the app is fully usable when stationary.

---

## Navigation Model

Beyond line-by-line stepping, the actor needs to jump around the script efficiently.

### Levels of navigation

1. **Line level** — previous / next actor line (voice: "back", "next")
2. **Part/scene level** — jump to first actor line in a part or scene (voice: "go to scene 2"; UI: scene list)
3. **Play section level** — jump to first actor line in a higher-level section if the manifest exposes one

### Bookmark

The actor can drop a bookmark at the current position (voice: "bookmark this"; UI: star button). Bookmarks are persisted in MMKV and shown in the script browser. Tapping a bookmark resumes from that line.

### Resume on launch

When the app launches with an active session, it restores `currentLineId` and offers "Resume" (start from saved position) or "Start over" (return to first actor line).

---

## Screens

### 1. Library

The home screen. Lists installed scriptpacks. Each entry shows play title, package source or company when provided, and the actor's selected role if one has been saved.

Actions:
- Tap a scriptpack → Role select (or directly to Session if role already chosen)
- "Import scriptpack" button → `expo-document-picker` to select a `.zip` from Files/Downloads
- Swipe to delete a scriptpack (with confirmation)

### 2. Role select

Shown after tapping a scriptpack for the first time, or when the actor wants to switch role.

Displays the manifest `roles` array as a list. Tap to select and proceed to Session setup.

### 3. Session setup

Shown before starting or resuming a session. Lets the actor configure:

- **Cue depth**: how many preceding lines to hear as cue (1–5; default 1)
- **Include stage directions in cue**: toggle (default on)
- **Start position**: "From beginning", "Resume", or "Choose scene"

"Start rehearsal" button launches the Session screen and begins audio playback.

### 4. Session (primary screen)

The main rehearsal screen. Designed to be glanceable — large text, high contrast, minimal controls.

**Display (top half):**
- Current position: "Act II · Scene 1 · Line 14 of 47"
- Cue text (the lines just played) — shown in muted colour
- Your line text — hidden by default; revealed on demand by tap

**Controls (bottom half):**

```
[ ◀ Back ]   [ ▶ Repeat cue ]   [ Skip ▶▶ ]

         [ Hear my line ]

              [ Next ▶ ]
```

**Driving mode toggle** (accessible from session setup or via a persistent button): collapses the screen to a minimal display — large position indicator, no text content, a single "Stop" button. All other interaction is voice-only.

**Status indicator**: a small persistent indicator showing whether voice recognition is active (listening for commands).

### 5. Script browser (modal/sheet)

Accessible from the Session screen via a menu button. Shows the full act/scene structure with progress indicators (which scenes have been rehearsed in this session). Tap any scene to jump to it.

---

## Driving Mode

When driving mode is active:

- Screen dims to a minimal display (position info + "Stop" button only)
- Voice command recognition is the only input method
- The app registers as a background audio session (behaves like a podcast/audiobook app)
- On iOS: use `AVAudioSession` category `playAndRecord` with `allowBluetooth` and `defaultToSpeaker` options
- On Android: register a foreground service with audio focus to prevent the OS from killing playback
- Bluetooth headsets and car audio are supported natively via the OS audio routing

The actor should be prompted to enable driving mode before starting a session if the phone detects it is connected to a car Bluetooth device (optional enhancement).

---

## Data Persistence (MMKV)

All state is local. No cloud sync.

| Key | Value | Description |
|---|---|---|
| `scriptpacks` | `string[]` | List of installed scriptpack IDs |
| `scriptpack:{id}:manifest` | `object` | Cached parsed manifest |
| `scriptpack:{id}:path` | `string` | Absolute path to unpacked folder |
| `session:{id}:role` | `string` | Selected role for this scriptpack |
| `session:{id}:position` | `string` | Current line ID |
| `session:{id}:config` | `object` | Cue depth, direction toggle, etc. |
| `session:{id}:bookmarks` | `string[]` | Bookmarked line IDs |
| `session:{id}:history` | `object` | Per-line attempt counts (optional) |

---

## Scriptpack Authoring (Out of Scope for App)

The app consumes scriptpacks but does not create them. Authoring is a separate workflow for the production:

1. Prepare an Androcles play under `plays/<play_id>/`.
2. Generate role markdown and special-role scripts with `./main text`.
3. Record role, narrator, caller, and optional announcer audio.
4. Split recordings into `build/<play>/audio/segments/<ROLE>/<segment_id>.wav`.
5. Generate the app package and `manifest.json` using the planned app-package CLI command.
6. Zip `build/<play>/app/` and distribute it by file transfer, CDN link, QR code in the rehearsal pack, etc.

The Androcles CLI is the initial scriptpack authoring tool. A web-based authoring app could be a later product, but it should emit the same versioned manifest contract.

---

## Out of Scope (v1)

The following are deliberate exclusions for the initial version:

- Line scoring or speech analysis
- Cloud sync or multi-device support
- Social / cast sharing features
- In-app scriptpack store or payments
- Scriptpack authoring UI
- CarPlay / Android Auto integration (desirable but complex; defer to v2)
- Video or self-tape recording

---

## Open Questions for Implementation

1. **STT permission handling**: Both iOS and Android require explicit user permission for microphone and speech recognition. The onboarding flow should explain why this is needed (voice commands only, nothing is recorded or transmitted) and gracefully degrade to tap-only if permission is denied.

2. **Scriptpack size limits**: No hard limit is imposed by the app, but the UI should show storage usage and warn if a scriptpack exceeds 500MB.

3. **Audio interruption handling**: Phone calls, notifications, and other audio interruptions should pause the session and allow resumption. Use `expo-av`'s interruption handling callbacks.

4. **Scriptpack versioning**: If a production updates the scriptpack (e.g. a line changes during rehearsals), the app should detect a version mismatch and prompt the actor to re-download. The manifest should include package-version metadata before this behavior is implemented.

5. **Accessibility**: The session screen should be fully usable with VoiceOver (iOS) and TalkBack (Android) for actors with visual impairments, though the voice command system largely serves this need already.
