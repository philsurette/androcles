...
This plan prepares the current CLI codebase for reliable actor-rehearsal app asset generation. It is written as a resumable checklist: complete each item, commit in small logical chunks, and leave notes under the relevant section if work is paused.

## Goals

- Preserve the existing CLI workflows for markdown, segment splitting, verification, timings, cues, and audioplay.
- Make cue generation correct, tested, and suitable as the foundation for an actor line-learning workflow.
- Introduce a stable app-facing export boundary rather than requiring a mobile app to infer structure from markdown, spreadsheets, or MP4 chapter metadata.
- Reduce coupling in the CLI so future app packaging can reuse domain services directly.
...
- There is no versioned app manifest or app package format.
- `src/build.py` is too large and mixes Typer CLI concerns with orchestration and domain behavior.
...
- [ ] Define whether caller/announcer snippets are included in actor rehearsal packages.
...
- [ ] Proposed services: `TextArtifactBuilder`, `SegmentBuildService`, `TimingBuildService`, `AudioPlayBuildService`, `CueBuildService`, `AppPackageBuilder`.
- [ ] Define which services may call ffmpeg, pydub, Whisper, or Audacity.
- [ ] Define which services are pure enough for fast tests.
- [ ] Define CLI ownership: Typer validates options and delegates to services; services raise exceptions.
...
- [ ] Required: role response segments used in app packages and audioplay.
...
- [ ] Define CLI flags for diagnostic mode, e.g. `--allow-missing-audio`.
...
- [ ] Verify `run_cues()` still writes cue files through the CLI.
...
## Phase 2: Add App Manifest Domain Model
...
- [ ] Add JSON serialization via a class method or writer class, not ad hoc dict construction in the CLI.
...
- [ ] Run targeted app manifest tests.
- [ ] Run full tests.

## Phase 3: Build App Package Exporter

Target outcome: `build/<play>/app/manifest.json` can be generated and consumed by a future mobile app.

- [ ] Create `src/app_package_builder.py`.
- [ ] Inject `Play`, `PathConfig`, and any audio-length helper.
- [ ] Build package directory at `build/<play>/app`.
- [ ] Write `manifest.json` with paths relative to the app package root.
...
## Phase 4: Add CLI Command For App Export

Target outcome: the future app package is generated by an explicit CLI command without bloating `src/build.py` further.

- [ ] Add an `app` or `package-app` command in `src/build.py`.
- [ ] Keep Typer code thin: parse options, build `PathConfig`, call `AppPackageBuilder`.
- [ ] Support `--play/-p`.
...
- [ ] Add CLI-level tests if the existing suite has a Typer test pattern; otherwise test the service directly.
- [ ] Run `./main --help` manually and confirm command appears.
- [ ] Run `./main package-app --help` or the final command equivalent.
- [ ] Run full tests.

## Phase 5: Enforce Missing Audio Policy

Target outcome: builds that produce user-consumable audio/app packages do not silently succeed with zero-length placeholders.

- [ ] Replace zero-length clip behavior in app package generation with exceptions.
...
## Phase 6: Extract Build Services From CLI

Target outcome: Typer commands become thin wrappers over reusable services.
...
- [ ] Keep `src/build.py` focused on Typer option parsing and service invocation.
...
Target outcome: app-oriented services and tests do not depend on import-time global play configuration.
...
- [ ] Add tests that construct two `PathConfig` instances for different plays in the same process and verify no cross-contamination in app export.
- [ ] Run full tests.
...
- [ ] Keep `typer.echo()` in CLI code for deliberate user-facing output.
...
- [ ] Add CLI examples for app package generation.
- [ ] Add a short `planning/app_package_usage.md` or update an existing user-facing doc with generated package layout.
- [ ] Document the test command for app package fixtures.
- [ ] Document any expected external dependencies for app package generation, such as ffmpeg if audio transcoding is used.
- [ ] Run `./main text` for the configured play if text output behavior changes.
- [ ] Run any new app package command against the configured play if fixture audio is available.

## Suggested Commit Slices

- [ ] Commit 1: design docs under `planning/`.
- [ ] Commit 2: `CueBuilder` correctness fix and tests.
- [ ] Commit 3: app manifest dataclasses and JSON writer tests.
- [ ] Commit 4: app package builder and service tests.
- [ ] Commit 5: CLI command for app package export.
