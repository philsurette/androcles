...
Adding optional blocking diagram JSON assets is a backward-compatible Playbook minor-version addition. Playbooks that include the root `staging` field and `staging/` package files should use `format_version: 1.1.0` or newer. Existing Cuemaster clients that only understand `1.0.0` may warn for the newer minor version and ignore `staging` while preserving all v1 rehearsal behavior.
...
- `format_version`: Semantic Playbook package format version.
- `package_type`: `playbook`.
...
- `staging`: Optional blocking diagram asset entry point. Present only when the Playbook includes blocking diagram JSON assets.
...
## Blocking Diagram Assets

Blocking diagram assets let Cuemaster show stage/blocking diagrams without parsing `production.md`, the exported staging overlay, or Stager resolver internals. They are optional Playbook assets and must not be required for basic rehearsal playback.

When included, the root `staging` object points at a package-local diagram bundle manifest:
...
- `included`: `true` when staging assets are packaged. If staging assets are excluded, omit the root `staging` object rather than writing `included: false`.
- `format`: `quince.blocking.diagram_bundle`.
- `format_version`: Semantic version for the staging diagram bundle contract. This is separate from the Playbook package `format_version`.
- `manifest_path`: Relative path from `manifest.json` to the staging diagram bundle manifest.

The bundle manifest describes compact renderer input, not SVG output:
  ...
  "checkpoints": [
    {
      ...
      "scene_id": "1.3",
      "beat_id": null,
      "production_anchor": "1.3-1",
      "path": "staging/checkpoints/scene-1.3-start.json"
    }
  ],
  "deltas": [
    {
      ...
      "scene_id": "1.3",
      "beat_id": "b2",
      "production_anchor": "1.3-3",
      "from_checkpoint": "scene:1.3:start",
      "path": "staging/deltas/scene-1.3-b2.json"
    }
  ]
}
```
...
Initial delta operations should be deliberately small:
...
- Playbook generation should include blocking diagram assets by default when staging data exists.
- A Playbook build option may exclude these assets; when excluded, omit the root `staging` object and the `staging/` package directory.
- Cuemaster should ignore the root `staging` object if it does not support blocking diagrams.
- Cuemaster must not parse `production.md` or `staging.txt` from a Playbook.
- Stager should validate every referenced checkpoint and delta path before packaging.
- Checkpoints and deltas should use stable scene ids, beat ids, and production anchors so Cuemaster can associate diagrams with script lines.
- SVG files are not the Playbook storage format. SVG remains a renderer output for the Block CLI and can be regenerated from diagram state.
