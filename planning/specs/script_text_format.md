...
Future source formats may be added if they can compile into canonical `production.md`.

### Production Scripts

`plays/<play_id>/production.md` is the canonical normalized script. It is generated by Stager, remains editable with ordinary text editors, and is the source of stable production ids.
...
- `draft`: Stager may regenerate `production.md` from `play.txt`.
- `locked`: Stager must not overwrite `production.md` except through an explicit force or reconciliation command.
...
Production entries have these shapes:
