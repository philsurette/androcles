...
- the canonical `production.txt` format used after script hardening,
...
`plays/<play_id>/play.txt` is a draft/source script. Stager may support multiple source formats over time.
...
Future source formats may be added if they can compile into canonical `production.txt`.

### Production Scripts

`plays/<play_id>/production.txt` is the canonical normalized script. It is generated by Stager, remains editable with ordinary text editors, and is the source of stable production ids.

The production format is line-oriented:

- each addressable script unit occupies exactly one physical line,
- every non-comment, non-blank script line starts with a production id,
...
Required `production.txt` metadata:
...
- `draft`: Stager may regenerate `production.txt` from `play.txt`.
- `locked`: Stager must not overwrite `production.txt` except through an explicit force or reconciliation command.
...
A production script line has this shape:
...
[@DIRECTION]: <stage direction text>
[<ROLE>]: <spoken text>
[<ROLE>,<ROLE>]: <simultaneous spoken text>
```
...
- Square brackets are used for entry tags, so they should not also delimit inline directions.
...
- a non-comment, non-blank line without a production id,
