...
The existing numeric `schema_version` field remains in v1 manifests for compatibility with older tooling. New package writers emit both fields:
...
For Stager-side import helpers, `schema_version: 1` may still be treated as `format_version: 1.0.0` when `format_version` is absent. Browser consumers should require `format_version` so packages without an explicit semantic format version fail with a clear validation error.
...
The Playbook manifest includes:
...
`package_type` gives consumers a cheap validation point even when the zip layout already implies the type.
...
Recording Request manifests include:
...
LineRecorder recording export package manifests include:
...
The current recording contracts are defined by [recording_package_manifest.md](recording_package_manifest.md).
...
The publication id is generated by Stager when publishing. It is a 12-character lowercase Base32-style token generated with enough entropy to make accidental collisions negligible in local production workflows, such as `k9f4p2x8m1qd`. Eight hex characters is too short because it only carries 32 bits of entropy. A full UUID, UUIDv7, or ULID is also acceptable internally, but the manuscript-facing id should be short enough for humans to compare and type when needed.
...
This means two producers probably started from the same prior version and independently published "version 7". Stager detects and reports this instead of treating one as a normal successor of the other.
...
When Stager sees a fork or out-of-date publish attempt, it stops publication and asks the producer to merge or intentionally publish a new successor version.
...
Production version ids are created by manuscript publication, not by Playbook generation. Building a Playbook, Recording Request, audioplay, or other artifact must not mutate `production.md`; those commands consume the selected production source and copy its production metadata into their outputs.
...
4. Verify the working `production_version` matches the current published version when history exists, preventing out-of-date publish attempts.
5. Collect a producer-authored change summary.
   - Interactive CLI use may prompt for it.
   - Non-interactive use must pass `--change-summary`.
   - Empty summaries are allowed only with `--allow-empty-summary`.
...
When `production-diff` detects differences between the working file and the current published snapshot, it reports:
...
Playbooks include the production version they were built from:
...
Recording Requests include the production version they were created from:

```json
{
  "request": {
    "id": "androcles-MEGAERA-selected-2026-05-17",
...
LineRecorder recording export packages preserve the originating request's production version:
...
Stager import compares the package production version to the target published production version and warns on mismatches. The content hashes remain the decisive stale-recording check because a production version mismatch can still contain unchanged segments.
...
Working-source builds preserve the source `production.version` when present and mark `source: "working"`. Stager does not create synthetic working production versions.

Locked `production.md` files may omit `production_version` before a play has publication history. Once publication history exists, publish attempts require the working source to carry the current published production version so divergent edits can be detected.
