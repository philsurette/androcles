...
- Missing segment audio was previously logged and represented as zero-length clips in audioplay plans; audioplay and Playbook planning now fail on missing required audio.
...
- [x] Replace zero-length clip behavior in Playbook generation with exceptions.
- [x] Review [play_plan_builder.py](../../src/stager/audiobook/play_plan_builder.py) missing snippet handling.
- [x] Decide whether audioplay should also fail by default or preserve current diagnostic behavior behind a flag.
- [x] Do not add `allow_missing_audio` plumbing to audioplay; audioplay is user-consumable output and should fail by default.
- [x] Add tests for missing role segment audio.
- [x] Add tests for missing simultaneous segment audio.
- [x] Add tests for missing callout audio when callouts are enabled.
- [x] Add tests for missing announcer/Librivox snippets when Librivox is enabled.
- [x] Run full tests.
