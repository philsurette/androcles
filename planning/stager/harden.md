...
- [ ] Represent required cue and response audio explicitly; do not emit rehearsable Playbook lines with missing audio.
...
- [ ] Fail on missing required role cue or response audio by default.
- [ ] Support diagnostic mode only if defined in `planning/stager/missing_audio_policy.md`.
...
- [ ] Add a `playbook` or `package-playbook` command in `src/stager/cli/build.py`.
